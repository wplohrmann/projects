...
def RunCases():
  ...
  for case_number in range(NUM_CASES):
    ...
    # n_clusters = 1# random.randint(0, 100)
    # cluster_connectivity = 50000

    # degrees = (
    #   [cluster_connectivity for _ in range(n_clusters)] +
    #   [random.randint(1, 1) for _ in range(N-n_clusters)]
    # )
    degrees = [random.randint(1, int(1+i/10000)) for i in range(N)]
    if sum(degrees) % 2 == 1:
      degrees[random.randint(0, N-1)] += 1 # Must have an even number of stubs

    stubs = [i for i, degree in enumerate(degrees) for _ in range(degree)]
    correct_total_edges = len(stubs) // 2
    random.shuffle(stubs)
    adj = [[] for _ in range(N)]
    for i in range(0, len(stubs), 2):
      stub1 = stubs[i]
      stub2 = stubs[i + 1]
      # TODO: Self-connections not allowed
      adj[stub1].append(stub2)
      adj[stub2].append(stub1)
